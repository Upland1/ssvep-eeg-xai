"""Stratified 5-Fold Cross-Validation Benchmark for Compact-CNN on Raw SSVEP Windows.

Mirrors benchmark_eegnet.py exactly (same quality gate, same CV scheme,
same smoothing scoring, same training procedure) so the two architectures
are compared on equal footing -- only the model differs.
"""
...
parser = argparse.ArgumentParser(description="Benchmark Compact-CNN on raw SSVEP EEG segments.")
...
# 2. Two-tier artifact quality (identical to benchmark_eegnet.py): drop
#    chronically bad channels for this subject first, then reject
#    remaining noisy windows on the surviving channels.
...
for fold_idx, (train_idx, test_idx) in enumerate(skf.split(windows_clean, y_mapped, groups=trial_ids_clean), 1):
    ...
    # benchmark_eegnet.py, for a like-for-like comparison).
    ...
    # Tier 1 channel dropping for THIS subject -- Compact-CNN's spatial
    ...
    model = CompactCNN(
        ...
        fs=250.0,
        lowest_freq_hz=LOWEST_TARGET_FREQ_HZ,
        dropout_rate=0.5,
    ).to(args.device)
...
print("\n" + "=" * 70)
print(f"          Compact-CNN BENCHMARK RESULTS (SUBJECT {args.subject})")
print("=" * 70)
